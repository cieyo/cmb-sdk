...
func TestParseNotification_PaymentResult(t *testing.T) {
	client := mustNewTestClient(t)
	corpNbr := "P0204921"

	// FINS Example
	notdat := `{"msgtyp":"FINS","msgdat":{"trsInfo":{"reqNbr":"5000021712","yurRef":"202458260078114775","trsAmt":"888.88","reqSts":"FIN","rtnFlg":"S"}}}`
	payload := activeNotificationPayload{
		SigTim: "20260305171000",
		SigDat: "debug",
		NotDat: notdat,
		NotTyp: "YQN02030",
	}
	signedPayload, _ := json.Marshal(payload)
	ciphertext, _ := encryptCTRBase64([]byte(client.config.SM4Key), corpNbr, signedPayload)
	env := activeNotificationEnvelope{CorpNbr: corpNbr, EnptData: ciphertext}
	body, _ := json.Marshal(env)

	msg, err := client.ParseNotification(body)
	if err != nil {
		t.Fatalf("ParseNotification (FINS) failed: %v", err)
	}
	if msg.MsgTyp != "FINS" || msg.MsgDat.TrsInfo == nil || msg.MsgDat.TrsInfo.YurRef != "202458260078114775" {
		t.Fatalf("unexpected msg: %+v", msg)
	}
	if !msg.MsgDat.TrsInfo.IsSuccess() {
		t.Fatal("expected success")
	}

	// FINB Example
	notdatB := `{"msgtyp":"FINB","msgdat":{"backInfo":{"reqNbr":"5000021712","yurRef":"202458260078114775","trsAmt":888.88,"rtnNar":"账户户名不符"}}}`
	payload.NotDat = notdatB
	signedPayload, _ = json.Marshal(payload)
	ciphertext, _ = encryptCTRBase64([]byte(client.config.SM4Key), corpNbr, signedPayload)
	env.EnptData = ciphertext
	body, _ = json.Marshal(env)

	msgB, err := client.ParseNotification(body)
	if err != nil {
		t.Fatalf("ParseNotification (FINB) failed: %v", err)
	}
	if msgB.MsgTyp != "FINB" || msgB.MsgDat.BackInfo == nil || msgB.MsgDat.BackInfo.YurRef != "202458260078114775" {
		t.Fatalf("unexpected msg: %+v", msgB)
	}
}
